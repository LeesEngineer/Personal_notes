...
<p>结论：按照 wi + si 从小到大的顺序排序，最大的危险系数一定是最小的</p>

<p>证明：</p>

1. 贪心得到的 res >= ans：res 是一种方案，故成立

2. res <= ans：假设最优解不是按该方法从小到大排序的，一定存在相邻两头牛使得 wi + si > w(i+1) + s(i+1)，交换两头牛，不影响其他所有牛的值。

|       |         第 i 个位置上的牛          |       第 i+1 个位置上的牛       |
|-------|---------------------------------|-------------------------------|
| 交换前 |   w1 + w2 + ... + w(i-1) - si   |  w1 + w2 + ... + wi - s(i+1)  |
| 交换后 | w1 + w2 + ... + w(i-1) - s(i+1) |  w1 + w2 + ... + w(i+1) - si  |
...
<p><b> wi + si 严格大于 si ，所以交换后两项的最大值严格小于 wi + si ，也就小于 交换前两项的最大值</b>，所以只要存在前一个大于后一个的情况，就有交换后两项的最大值一定会严格变小</p>

<p><b>因此只要存在一个逆序，把这两个数交换，其余所有数都不变，这两个数的最大值还可以变小，那么所有数的最大值起码不会变大（要么变小要么相等）</b>，所以最优解不是严格从小到大递增的，那么一定可以通过这种方式，把他变成从小到大递增的序列，而且最大值不会变大。</p>
